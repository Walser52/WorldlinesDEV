...
# Integrating Factor (Special Cases)

- If μ only depends on **x**: $\mu_y = 0$
  $$
  \mu = \exp\left(\int \frac{\partial N/\partial x - \partial M/\partial y}{M} \; dx\right)
  $$
  Must be a function of **x** only.

- If μ only depends on **y**: $\mu_x = 0$
  $$
  \mu = \exp\left(\int \frac{\partial M/\partial y - \partial N/\partial x}{N} \; dy\right)
  $$
  Must be a function of **y** only.
...
- If you can take **x** common from one part and **y** common from another:
  $$
  \mu = \frac{1}{M_y - N_x}
  $$
...
- If $n = 0$ or $n = 1$: directly solvable

## 3. Reduction to Separation of Variables
...
- Check if homogeneous or exact:
  - If homogeneous: use substitution to convert into separable form
  - If not homogeneous: check for exactness
  - If not exact: make it exact using integrating factor
...
{{< include _figs/mermaid_diagram_flow.qmd >}}